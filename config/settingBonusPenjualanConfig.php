...
if(isset($_POST['submit'])){
    ...
    if(($umrohDP == 0 || $umrohDP == "" ) || ($HajiDP == 0 || $HajiDP == "" )){
        ...
        $saveHarga = $hargaBonusClass->UpdateHargaBonus("PENJUALAN",$umrohDP,$HajiDP);
        if($saveHarga){
            $_SESSION['alertSuccess'] = "Data tersimpan.";
            header('Location: setting-bonus-penjualan');
            exit();
        }
    }
}
...
function nominalHarga(){
    ...
    $data = $hargaBonusClass->selectHargaBonus("PENJUALAN");
